...
public class Sliceable : MonoBehaviour, ISliceable
{
    ...
    [HideInInspector]
    public SlicingSystem slicingSystem;
    ...
    public void SliceByLine(Camera camera, Vector3 start, Vector3 end)
    {
        if (currentlySliceable)
        {
            TurboSlicerSingleton.Instance.SliceByLine(gameObject, camera, start, end, true);
        }
    }

    private void OnEnable()
    {
        Sliced += RegisterWithSliceSystem;
    }
    ...
    private void OnDisable()
    {
        if (slicingSystem)
        {
            slicingSystem.objectsToCut.Remove(gameObject);
        }
    ...
    private void Start()
    {
        if(slicingSystem == null)
        {
            slicingSystem = FindObjectOfType<SlicingSystem>();
            slicingSystem.objectsToCut.Add(gameObject);
        }
    }

    void RegisterWithSliceSystem(object sender, NobleMuffins.TurboSlicer.SliceEventArgs e)
    {
        foreach(var newSlice in e.Parts)
        {
            if (slicingSystem)
            {
                newSlice.GetComponent<Sliceable>().slicingSystem = slicingSystem;
            }

            slicingSystem.objectsToCut.Add(newSlice);
        }
    }
