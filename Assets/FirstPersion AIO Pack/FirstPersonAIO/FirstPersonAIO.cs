...
public class BETA_SETTINGS{
    ...
    private void FixedUpdate()
    {
        ...
        {
            RaycastHit[] hits = Physics.SphereCastAll(transform.position - new Vector3(0, ((capsule.height / 3) * transform.localScale.y) - capsule.radius, 0), capsule.radius, Vector3.down, capsule.radius, Physics.AllLayers, QueryTriggerInteraction.Ignore);
            float nearest = float.PositiveInfinity;
            IsGrounded = false;
            //Debug.Log(hits.Length);

            //Vector3 sphereCastPosition = transform.position - new Vector3(0, ((capsule.height / 2) * transform.localScale.y) - 0.01f, 0);

            DebugExtension.DebugWireSphere(transform.position - new Vector3(0, ((capsule.height / 3) * transform.localScale.y) - capsule.radius, 0), Color.yellow, capsule.radius);
